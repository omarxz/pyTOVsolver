...
def continuity_cost(a_initial_guess, rho_c):
    ...
    if a_R >= 0 or a_out[0] >= 0:
        print(f"[{os.getpid()}] a_R >= 0 or a_out[0] >= 0. Cost = np.inf")
        return  np.inf
    else:
        cost_a_R = (a_R - a_out[0])**2
...
def compute_for_rho_c(rho_c, g_s_N=g_s_N):
    ...
    result = minimize(continuity_cost, a_minimized, args=(rho_c,), method='Nelder-Mead', options={'maxiter': 150, 'xatol': 1e-9}) # OR maybe 150 is too much? default is 100
    if result.success: 
